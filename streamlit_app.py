...
# 4. ฟังก์ชันแปลงวินาทีและจัด Format เวลา (ตัด 00: นำหน้าออก)
def format_seconds_to_time(total_seconds):
    hours = int(total_seconds // 3600)
    minutes = int((total_seconds % 3600) // 60)
    seconds = int(total_seconds % 60)
    
    # ถ้าเวลาไม่ถึงชั่วโมง ให้แสดงแค่ mm:ss (ตัด 00: ทิ้งเหมือน Datapaq)
    if hours == 0:
...
if uploaded_files:
    ...
    if df.empty:
        ...
        # ---------------------------------------------------------
        # 📊 ตารางสรุปค่า (แก้ไขขอบเขตเวลาและลำดับ Probe แล้ว)
        # ---------------------------------------------------------
        st.markdown("### 📊 ตารางสรุปผลการวิเคราะห์ (Data Table for Google Sheets Copy)")

        # 📌 ปรับช่วงเวลาประมวลผลให้ตรงกับ Datapaq 100% ครบทุกชุดข้อมูล
        # Dryer Zone (Above 175°C): 0 ถึง 270 วินาที
        dryer_subset = df[(df["ElapsedSeconds"] >= 0) & (df["ElapsedSeconds"] <= 270)]
        
        # Debinder Zone (Above 200°C): 330 ถึง 840 วินาที
        debinder_subset = df[(df["ElapsedSeconds"] >= 330) & (df["ElapsedSeconds"] <= 840)]
        
        # Brazing Zone Dwell Time: สะสมเวลาทั้งกราฟ (0 ถึง 2200 วินาที)
        brazing_ht_subset = df[(df["ElapsedSeconds"] >= 0) & (df["ElapsedSeconds"] <= 2200)]
        
        # Brazing Zone Max Temp: ช่วงแช่อุณหภูมิสูงสุด (900 ถึง 1750 วินาที)
        brazing_max_subset = df[(df["ElapsedSeconds"] >= 900) & (df["ElapsedSeconds"] <= 1750)]

        # 📌 กำหนดลำดับ Probe ให้ถูกต้องตามภาพ: 1, 2, 3, 8, 4, 5, 6, 7
        ...
        for p_num, col_name in ordered_cols:
            # 📌 การจัดกลุ่มตำแหน่ง (Position Name) ตามที่เลือกจาก Sidebar
            ...
            # นับจำนวนวินาทีที่อุณหภูมิถึงเกณฑ์
